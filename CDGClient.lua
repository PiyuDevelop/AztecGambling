...
function CDGClient:RegisterCallbacks()
	...
	self:RegisterComm("CDG_TURN_UPDATE", "TurnUpdateCallback")
...
function CDGClient:TurnUpdateCallback(...)
	if (self.current_game == nil) then return end
	...
	if (self.current_game.turn_player == "RollOff") then
		self.ui.CDG_Frame:SetStatusText("Roll-off to go first: Roll "..self.current_game.roll_range)
	else
		self.ui.CDG_Frame:SetStatusText(self.current_game.turn_player.."'s turn: Roll "..self.current_game.roll_range)
	end
end
