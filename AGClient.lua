...
function AGClient:OnInitialize()
	AztecGambling:PrintDebug("Load Begin") 
    ...
    self.db = LibStub("AceDB-3.0"):New("AGClientDB", defaults)
	self:ConstructUI()
	self:RegisterCallbacks()

	AztecGambling:PrintDebug("Load Complete!!")
end

-- Slash Command Setup and Calls
-- =========================================================
function AGClient:RegisterCallbacks()
	self:RegisterEvent("CHAT_MSG_SYSTEM", "RollCallback")
	
    -- callbacks to get game information from master
    self:RegisterComm("AG_NEW_GAME", "NewGameCallback")
    self:RegisterComm("AG_END_GAME", "GameResultsCallback")
	self:RegisterComm("AG_GUILD_ROLL", "GuildRollCallback")
	self:RegisterComm("AG_TURN_UPDATE", "TurnUpdateCallback")
	
	AztecGambling:PrintDebug("REGISTRATIONS COMPLETE")
end
...
function AGClient:RollCallback(...)
	if (self.current_game == nil) then return end

	-- Parse the input Args 
	local roll_text = select(2, ...)
	local message = AztecGambling:SplitString(roll_text, "%S+")
	...
	if valid_roll and valid_source then 
        self:SendCommMessage("AG_GUILD_ROLL", roll_text, self.current_game.addon_const, tostring(AztecGambling.db.global.custom_channel.index))
	end
...
function AGClient:GuildRollCallback(...)
	if (self.current_game == nil) then return end
...
function AGClient:NewGameCallback(...)
    -- Reset Game Settings -- 
	AztecGambling:PrintDebug("NEWGAME")
	local callback = select(1, ...)
	local message = select(2, ...)
	local chat = select(3, ...)
	local sender = select(4, ...)
	message = AztecGambling:SplitString(message, "%S+")
	...
	if self.db.global.auto_pop and valid_source then
		...
		self.ui.AG_Frame:SetStatusText(new_game_msg)
	end
	
	AztecGambling:PrintDebug(self.current_game.roll_lower)
	AztecGambling:PrintDebug(self.current_game.roll_upper)
	AztecGambling:PrintDebug(self.current_game.channel_const)
	AztecGambling:PrintDebug(self.current_game.roll_range)
end

-- Countdown: keep the roll range in sync as the turn/ceiling changes each roll
function AGClient:TurnUpdateCallback(...)
	if (self.current_game == nil) then return end

	local message = select(2, ...)
	message = AztecGambling:SplitString(message, "%S+")

	self.current_game.turn_player = message[1]
	self.current_game.roll_lower = message[2]
	self.current_game.roll_upper = message[3]
	self.current_game.roll_range = "("..self.current_game.roll_lower.."-"..self.current_game.roll_upper..")"

	if (self.current_game.turn_player == "RollOff") then
		self.ui.AG_Frame:SetStatusText("Roll-off to go first: Roll "..self.current_game.roll_range)
	else
		self.ui.AG_Frame:SetStatusText(self.current_game.turn_player.."'s turn: Roll "..self.current_game.roll_range)
	end
end

function AGClient:GameResultsCallback(...)
	if (self.current_game == nil) then return end
	...
	self.ui.AG_Frame:SetStatusText(results_msg)
	
end

-- Button Interaction Callbacks (State and Settings)
-- ==================================================== 
function AGClient:RollForMe()
	if self.current_game then
		RandomRoll(self.current_game.roll_lower, self.current_game.roll_upper)
	end
end

function AGClient:EnterForMe()
	if self.current_game then
		SendChatMessage(AG_MESSAGES.JOIN_SIGNAL, self.current_game.channel_const, nil, AztecGambling.db.global.custom_channel.index)
	end
end

function AGClient:OpenTradeWinner()
	InitiateTrade(self.current_game.winner)
end

-- Lock Position for UI  
function AGClient:SaveFrameState()
	self.db.global.ui = AztecGambling:CopyTable(self.ui.AG_Frame.status)
end

-- UI ELEMENTS 
-- ==============
function AGClient:ShowUI()
    -- Set the location to where the master is
	self.ui.AG_Frame.frame:ClearAllPoints()
	self.ui.AG_Frame.frame:SetPoint("TOPRIGHT", AztecGambling.ui.AG_Frame.frame, "TOPRIGHT", 0, 0)
	self.ui.AG_Frame:Show()
	self.db.global.window_shown = true
end

function AGClient:HideUI()
	self.ui.AG_Frame:Hide()
	self.db.global.window_shown = false
	self:SaveFrameState()
end

function AGClient:ConstructUI()
	...
	for _, button_name in pairs(ag_ui_elements.button_index) do
		local button_settings = ag_ui_elements.buttons[button_name]
		...
		self.ui.AG_Frame:AddChild(self.ui[button_name])
	end
	
	if (self.db.global.ui ~= nil) then
		self.ui.AG_Frame:SetStatusTable(self.db.global.ui)
	end
